tableau.extensions.initializeAsync().then(() => {
    ...
    let worksheet = worksheets.find(ws => ws.name === "Projects");
  ...
  const edit = async event => {
    ...
    data = data.data[0];
    const index = data.columns.find(col => col.fieldName === "Project Id").index;
    const requestid = data.data[0][index].value;
    const url = `${window.location.origin}/request/${requestid}`;
    console.log(url);

    let closePayload = await tableau.extensions.ui.displayDialogAsync(url, null, {width: 800, height: 650});
